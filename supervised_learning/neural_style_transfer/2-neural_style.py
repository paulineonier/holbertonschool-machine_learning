#!/usr/bin/env python3
"""
Neural Style Transfer (NST) class with model loading
and Gram matrix computation.
"""
...
class NST:
    ...
    def __init__(self, style_image, content_image, alpha=1e4, beta=1):
        """
        Initialize NST instance and load VGG19 model.
    ...
    def scale_image(image):
        """
        Rescale an image to [0, 1] and max side = 512.

        Args:
            image (np.ndarray): input image (h, w, 3)

        Returns:
            tf.Tensor: scaled image (1, h_new, w_new, 3)
